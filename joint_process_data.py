...
class DataProcessor():
  ...
  def __init__(self,
    ...
    labels_1 = np.load('./processed_data/dec_intentions.npy', allow_pickle=True)
    labels_2 = np.load('./processed_data/jan_intentions.npy', allow_pickle=True)
    emails_1 = np.load('./processed_data/dec_bodies.npy', allow_pickle=True)
    emails_2 = np.load('./processed_data/jan_bodies.npy', allow_pickle=True)
  ...
  def removeByLabelIndex(self):
    ...
    # print(len(self.index_from_label))
  ...
  def removeByEmailIndex(self):
    ...
    for i, email_text in enumerate(self.emails):
      ...
      email_text = email_text.replace(u'\xa0',
                                      u' ').replace('&#8217;', '\'').replace(
                                          '\\r\\n', '\n').replace('\u3000', '')
      # originally empty emails: [639, 640, 641, 642, 643, 1367, 2887, 2888, 3283, 3284]
      try:
      ...
      # email_sents_sig = email_sents[sig_start_idx:]
      # pdb.set_trace()
      # print(email_sents_body, email_sents_sig)
      email_sents_body = removeGreeting(email_sents_body)
      # pdb.set_trace()
      # print(email_sents_body)
      length_ = len(email_sents_body)
      f = open("./processed_data/long_email_28.txt", "a")
    ...
    # self.labels_p = removeLabelByEmptyEmailIndex(self.labels, empty_email_index)
    ...
    np.save('./processed_data/NoLongEmail_label.npy', self.labels_p)
    np.save('./processed_data/NoLongEmail_email.npy', self.email_bodies)
