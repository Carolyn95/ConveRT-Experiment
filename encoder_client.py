...
class EncoderClient:
  ...
  def __init__(
    ...
    with self._session.graph.as_default():
      ...
      self._fed_contexts = tf.compat.v1.placeholder(shape=[None],
                                                    dtype=tf.string)
      self._fed_extra_contexts = tf.compat.v1.placeholder(shape=[None],
                                                          dtype=tf.string)
      self._fed_responses = tf.compat.v1.placeholder(shape=[None],
                                                     dtype=tf.string)
  ...
  def encode_sentences(self, sentences):
    ...
    return _batch_session_run(
        ...
        {self._fed_contexts: sentences},
        self._session,
        self._internal_batch_size,
        self._fed_contexts,
    )
  ...
  def encode_contexts(self, contexts, extra_contexts=None):
    ...
    if not self._use_extra_context:
      if extra_contexts is not None:
        raise ValueError("This encoder does not use extra contexts, but extra "
    ...
    for extra_context in extra_contexts:
      ...
      if self._use_extra_context_prefixes:
        extra_context = [
            "{}: {}".format(i, text) for i, text in enumerate(extra_context)
        ]
...
def _batch_session_run(
  ...
  assert all([
      batch_dim == batch_dims[0] for batch_dim in batch_dims
  ...
  feed_dict = {
      key: [value[i] for i in ordering] for key, value in feed_dict.items()
  }

  outputs = []
  start_time = time.time()
  last_log = start_time
  glog.info("Encoding {total_size} examples.")
  for i in range(0, total_size, batch_size):

    if (time.time() - last_log) > _LOG_EVERY_SECS:
      glog.info("Encoded {i} / {total_size}  ({i/total_size:.1%})")
      last_log = time.time()

    batch_feed_dict = {
        k: np.array(v[i:i + batch_size]) for k, v in feed_dict.items()
    }
    outputs.append(session.run(output_tensor, feed_dict=batch_feed_dict))
...
def cache_encodings(encoding_function, cache_size):
  ...
  def _cached_function(examples):
    ...
    if uncached_examples:
      ...
      for example, encoding in zip(uncached_examples, uncached_encodings):
    ...
    return np.array([encodings.get(example) for example in examples])
