"""
This script is for get training stats using ConveRT,
including memory usage and time consuming.

TODO: class classifier lack of implementation of encoder_client
"""
...
from keras.layers import Input, Lambda, Dense, Dropout, Reshape, Flatten, AveragePooling1D
...
class Classifier():
  ...
  def encode(self, x):
    return self.module.encode_sentences(x)

  def createModel(self):
    input_text = Input(shape=(1,), dtype='string')
    embedding = tf.map_fn(self.encode, elems=input_text)  #(input_text)
...
class VanillaConveRT():
  ...
  def createModel(self):
    ...
    embedding = Lambda(self.average, output_shape=(512,))(embedding)
    dense = Dense(256, activation='relu',
                  kernel_regularizer=l1(0.0001))(embedding)
    dense = Dropout(0.4)(dense)
    pred = Dense(self.n_labels, activation='softmax')(dense)
  ...
  # python -m memory_profiler experiment.py
  # @profile
  def train(self, filepath):
    try:
      os.mkdir(filepath)
...
if __name__ == '__main__':
  ...
  vc.consolidateResult(filepath='test')
  print('Overall Time: ', str(time.time() - start_time), 's')

  # c = Classifier(dp.train_sents, dp.train_labels_onehot, dp.valid_sents,
  #                dp.valid_labels_onehot)
  # c.createModel()
  # c.train(filepath='test')
  # c.consolidateResult()
  # pdb.set_trace()
